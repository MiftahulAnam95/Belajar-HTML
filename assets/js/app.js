window.HTMLLabApp = (() => {
  const data = window.HTMLLabData;
  const progress = window.HTMLLabProgress;
...
<html lang="id">
  ...
  const renderLessonCodePreview = (item) => `
    <div class="lesson-code-preview">
      <div class="lesson-preview-label"><i class="bi bi-eye"></i> Preview hasil HTML</div>
      <iframe
        class="lesson-preview-frame"
        title="Preview contoh kode ${escapeHTML(item.title)}"
        sandbox="allow-same-origin"
        loading="lazy"
        srcdoc="${escapeHTML(buildPreviewDocument(item.code))}"
      ></iframe>
    </div>`;
  ...
  const renderHomeDemo = (id = data.homeDemos[0]?.id) => {
    ...
    tabs.innerHTML = data.homeDemos
      ...
      .join("");
    code.innerHTML = renderCodeBlock(demo.code, `${demo.id}.html`);
    preview.srcdoc = buildPreviewDocument(demo.code);
  ...
  const renderLessonLearningLoop = (item) => `
    <section class="detail-block learning-loop-block">
      ...
      <div class="learning-loop-grid">
        ...
        <article>
          <span>2</span>
          <strong>Lihat contoh kecil</strong>
          <p>Jangan hafalkan semua tag. Perhatikan satu pola kode lalu lihat hasilnya.</p>
        </article>
        <article>
          <span>3</span>
          <strong>Ubah satu bagian</strong>
  ...
  const renderLessonCodeBridge = (item) => `
    <div class="code-bridge-grid">
      <div>
        ...
        ${renderCodeBlock(item.code, item.filename)}
      </div>
      <div>
    ...
    lessonDetail.innerHTML = `
      <article class="lesson-detail-card">
        ...
        <div class="lesson-detail-body">
          ...
          <section class="detail-block">
            <h3><i class="bi bi-code-square"></i> Contoh kode HTML</h3>
            <p class="mini-guidance">Baca dari kiri ke kanan: kode adalah instruksi, preview adalah hasil yang dilihat pengguna.</p>
  ...
  const initCssColorPicker = (cssInput, onChange) => {
    ...
    const presets = ["#FFFFFF", "#111827", "#F97316", "#EA580C", "#16A36A", "#0EA5B7", "#5865B8", "#DC4C64", "#FBBF24", "#22C55E", "#38BDF8", "#A855F7"];
  ...
  const handleHashLesson = () => {
    ...
    if (lessonId && data.lessons.some((lesson) => lesson.id === lessonId)) {
      ...
      if (lesson) document.title = `${lesson.title} - HTML Beginner Lab`;
      return;
    }
...
document.addEventListener("DOMContentLoaded", window.HTMLLabApp.init);
